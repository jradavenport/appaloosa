...
def paper2_plots(condorfile='condorout.dat.gz', debug=False,
    ...
    if oldplot:
        for k in range(crng.shape[0]):
            ...
            B_V_ts = analysis.getBV(mass[okclr][ts])
            age_ts = analysis.MH2008_age(B_V_ts, Prot_all[okclr][ts])
            ...
            for l in range(np.size(ts)):
                ...
                for i in range(0, len(star)):
                    ...
                    if len(ok) > 0:
                        ...
                        flare_tot[ok] = flare_tot[ok] + \
                                    (fdata.loc[star[i], 7:].values[ok] * np.float(fdata.loc[star[i], 2]))
                ...
                ffd_yerr = analysis._Perror(flare_tot[::-1], down=True) / dur_all[okclr][ts][l]
            ...
            fitts, covts = curve_fit(analysis.FlareEqn0, Xts, logR_tsstack[stackOKts], p0ts,
            ...
            plt.figure()
            y1 = analysis.FlareEqn0((np.arange(33, 37), np.array([1, 1, 1, 1])), *fitts)
            y2 = analysis.FlareEqn0((np.arange(33, 37), np.array([2, 2, 2, 2])), *fitts)
            y3 = analysis.FlareEqn0((np.arange(33, 37), np.array([3, 3, 3, 3])), *fitts)
            y4 = analysis.FlareEqn0((np.arange(33, 37), np.array([4, 4, 4, 4])), *fitts)
        ...
        plt.figure()
        plt.scatter(Rossby[okclr][pok], np.log10(Lfl_Lbol[okclr][pok] * analysis.Chi_fl(gi_all[okclr][pok])),
                    s=50, linewidths=0.5, edgecolors='k', alpha=0.85, c=mass[okclr][pok], cmap=cm.Spectral)
        cbar = plt.colorbar()
        cbar.set_label(r'Mass (M$_{\odot}$)')
        ...
        B_V_pok = analysis.getBV(mass[okclr][pok])
        age_pok = analysis.MH2008_age(B_V_pok, Prot_all[okclr][pok])
        ...
        plt.figure()
        plt.scatter(gi_all[okclr][pok], Prot_all[okclr][pok],
                    c=np.log10(Lfl_Lbol[okclr][pok] * analysis.Chi_fl(gi_all[okclr][pok])),
                    s=50, linewidths=0.5, edgecolors='k', alpha=0.85, cmap=cm.magma_r)
        cbar = plt.colorbar()
    ...
    p0 = (0., 0., -0.5,
          -1., 1., 10.)
    fit, cov = curve_fit(analysis.FlareEqn, X, logR_stack[stackOK], p0,
                         sigma=logRerr_stack[stackOK])

    modelStack = analysis.FlareEqn(X, *fit)
    ...
    y1 = analysis.FlareEqn((np.arange(33, 37), np.array([1, 1, 1, 1]), np.array([0.5, 0.5, 0.5, 0.5])), *fit)
    y2 = analysis.FlareEqn((np.arange(33, 37), np.array([2, 2, 2, 2]), np.array([0.5, 0.5, 0.5, 0.5])), *fit)
    y3 = analysis.FlareEqn((np.arange(33, 37), np.array([3, 3, 3, 3]), np.array([0.5, 0.5, 0.5, 0.5])), *fit)
    y4 = analysis.FlareEqn((np.arange(33, 37), np.array([4, 4, 4, 4]), np.array([0.5, 0.5, 0.5, 0.5])), *fit)
    ...
    ffig = plt.figure(figsize=(6, 5))
    ax = ffig.add_subplot(111)
    y1 = analysis.FlareEqn((np.arange(33, 37), np.array([1, 1, 1, 1]), np.array([1.0, 1.0, 1.0, 1.0])), *fit)
    y2 = analysis.FlareEqn((np.arange(33, 37), np.array([2, 2, 2, 2]), np.array([1.0, 1.0, 1.0, 1.0])), *fit)
    y3 = analysis.FlareEqn((np.arange(33, 37), np.array([3, 3, 3, 3]), np.array([1.0, 1.0, 1.0, 1.0])), *fit)
    y4 = analysis.FlareEqn((np.arange(33, 37), np.array([4, 4, 4, 4]), np.array([1.0, 1.0, 1.0, 1.0])), *fit)
    ...
    plt.figure(figsize=(6, 5))
    y1 = analysis.FlareEqn((np.arange(28, 37), np.log10(4600. * np.ones(9)), np.ones(9)), *fit)
    ...
    for i in range(mass_x.size):
        for j in range(age_y.size):
            rate_grid[i, j] = analysis.FlareEqn((35., age_y[j], mass_x[i]), *fit)
    ...
    R1s_fit = analysis.FlareEqn((Lkp_all[okclr][ts], np.log10(age_ts), mass[okclr][ts]), *fit)
    ...
    for i in range(mx.size):
        for j in range(age_y.size):
            R1s_grid[i, j] = analysis.FlareEqn((ly[i], age_y[j], mx[i]), *fit)
    ...
    if True:
        ...
        for l in range(np.size(ts)):
            ...
            for i in range(0, len(star)):
                ...
                if len(ok) > 0:
                    ...
                    plt.plot(edbins[1:][ok][::-1] + Lkp_i,
                             np.log10(np.cumsum(fdata.loc[star[i], 7:].values[ok][::-1]) + 1e-10),
                             # put 1e-10 buffer in for plot
            ...
            if len(ffd_ok[0]) > 1:
                ...
                model_y = analysis.FlareEqn((ffd_x[ffd_ok], np.log10(age_ts[l]), mass[okclr][ts][l]), *fit)
                ...
                plt.text(0.025, 0.025,
                         r'$\chi^2$=' + format(chisq_ts[l], '.2F') + ', g-i=' + format(gi_all[okclr][ts][l],
                                                                                       '.2F') + ', Prot=' + format(
                             Prot_all[okclr][ts][l]),
    ...
    if False:  # set to True to run for Riley
        ...
        for k in range(mass_wb.size):
            for l in range(age_range.size):
                ...
                # X = (logE, logT, M)
                ffd_kl = (10. ** analysis.FlareEqn((ed_wb + Lkp_wb[k], age_range[l] * np.ones(20), mass_wb[k] * np.ones(20)),
                                          *fit)) / (60. * 60. * 24.)
    ...
    B_V_nd = analysis.getBV(mass[oknd])
    age_nd = analysis.MH2008_age(B_V_nd, Prot_all[oknd])
    ...
    if False:
        ...
        for l in range(np.size(oknd)):
            ...
            # the "error" is the Poisson err from the total # flares per bin
            ffd_yerr = analysis._Perror(flare_tot[::-1], down=True) / dur_all[oknd][l]
        ...
        fit_nd, cov_nd = curve_fit(analysis.FlareEqn_nolog, X_nd, R_ndstack[stackOK_nd], p0,
                                   sigma=Rerr_ndstack[stackOK_nd])

        # res = minimize(FlareEqn, p0, method='BFGS', args=(X_nd, logR_ndstack[stackOK_nd]))

        modelStack_nd = analysis.FlareEqn_nolog(X_nd, *fit_nd)
        ...
        plt.figure(figsize=(6, 5))
        y1 = analysis.FlareEqn_nolog((np.arange(28, 37), np.log10(4600. * np.ones(9)), np.ones(9)), *fit_nd)
        ...
        R1s_fit_nd = np.log10(analysis.FlareEqn_nolog((Lkp_all[oknd], np.log10(age_nd), mass[oknd]), *fit_nd))
        ...
        for i in range(mx.size):
            for j in range(age_y.size):
                R1s_grid_nd[i, j] = np.log10(analysis.FlareEqn_nolog((ly[i], age_y[j], mx[i]), *fit_nd))
        ...
        if False:
            ...
            for l in range(np.size(ts)):
                ...
                if len(ffd_ok[0]) > 1:
                    ...
                    # make model FFD at same   ffd_x values
                    model_y = analysis.FlareEqn_nolog((ffd_x[ffd_ok], np.log10(age_ts[l]), mass[okclr][ts][l]), *fit_nd)

                    # compute chisq!
                    chisq_nd[l] = np.sum(((np.log10(ffd_y[ffd_ok]) - np.log10(model_y)) / yerr_temp) ** 2.0) / np.float(
                        model_y.size)
    ...
    if False:
        ...
        fit3, cov3 = curve_fit(analysis.FlareEqn_nolog, X, R_stack[stackOK], p0=fit,
        ...
        for i in range(mx.size):
            for j in range(age_y.size):
                R1s_grid_3[i, j] = np.log10(analysis.FlareEqn_nolog((ly[i], age_y[j], mx[i]), *fit3))

        p4 = (-1, -1, 0, 0, 0, 0, 1, 10)
        fit4, cov4 = curve_fit(analysis.FlareEqn2, X, logR_stack[stackOK], p0=p4,
                               sigma=logRerr_stack[stackOK])
        p5 = (-1, -1, 0, 0, 0, 0, 1, 10)
        fit5, cov5 = curve_fit(analysis.FlareEqn2_nolog, X, R_stack[stackOK], p0=p5,
                               sigma=Rerr_stack[stackOK])
        ...
        for i in range(mx.size):
            for j in range(age_y.size):
                R1s_grid_4[i, j] = (analysis.FlareEqn2((ly[i], age_y[j], mx[i]), *fit4))

        R1s_grid_5 = np.zeros((mx.size, age_y.size))
        for i in range(mx.size):
            for j in range(age_y.size):
                R1s_grid_5[i, j] = np.log10(analysis.FlareEqn2_nolog((ly[i], age_y[j], mx[i]), *fit5))
